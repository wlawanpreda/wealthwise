...
export function FinancialMilestones() {
  ...
  const { totalWealth, emergencyMonths, dti, savingsRate } = useDerivedFinancials();
  ...
  const milestones: Milestone[] = [
    ...
    },
    {
      ...
      icon: Star,
      isCompleted: totalWealth >= 1_000_000,
      color: "text-emerald-500",
    },
    {
      id: "4",
        ...
        {milestones.map((m, i) => {
          ...
          return (
            ...
            >
              ...
              <div
                className={cn(
                  "p-2 rounded-lg w-fit mb-3 bg-white shadow-sm border border-brand-border",
                  m.color,
                )}
              >
                <Icon size={16} />
              </div>
              <h4 className="text-[11px] font-black text-brand-text mb-1 uppercase tracking-tight">
                {m.name}
              </h4>
              <p className="text-[9px] font-bold text-brand-muted leading-tight">{m.description}</p>
            </motion.div>
          );
        })}
      </div>
